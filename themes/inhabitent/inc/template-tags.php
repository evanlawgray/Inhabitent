 ...
 function red_starter_entry_footer() {
 	...
 	if ( 'post' === get_post_type() ) {
 		...
 		if ( $categories_list && red_starter_categorized_blog() ) {
 			printf( '<span class="cat-links">' . 'Posted in <span class="arrow">&rarr;</span> %1$s' . '</span>', $categories_list ); // WPCS: XSS OK.
 		}

 		/* translators: used between list items, there is a space after the comma */
 		$tags_list = get_the_tag_list( '', esc_html( ', ' ) );
 		if ( $tags_list ) {
 			printf( '<span class="tags-links">' . 'Tagged <span class="arrow">&rarr;</span> %1$s' . '</span>', $tags_list ); // WPCS: XSS OK.
 		}
 	}
 }
